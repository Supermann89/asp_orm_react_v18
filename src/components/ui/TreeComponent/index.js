...
export function makeTree(nodes, parentId) {
  return nodes
    ...
    .reduce(
      (tree, node) => [
        ...
        {
          ...node,
          children: makeTree(nodes, node.id).sort((a, b) => {
...
export function flatTree(items) {
  var flated = [];
  for (var item of items) {
    ...
    let sub_flated = flatTree(item.children);
...
export const StyledEmptyTreeItem = styled((props) => <TreeItem {...props} />)(
  ({ theme }) => ({
    position: "relative",
    "&:before": {
      pointerEvents: "none",
